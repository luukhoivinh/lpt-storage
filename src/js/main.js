...
/* ── Subtitle link sync (canvas px → CSS px) ────────────── */
const subtitleLinkEl = document.getElementById("subtitleLink");
function syncSubtitleLink(canvasY, canvasFontSize) {
  if (!subtitleLinkEl) return;
  const dpr = renderer.getPixelRatio() || 1;
  const cssY = canvasY / dpr;
  const cssFontSize = canvasFontSize / dpr;
  subtitleLinkEl.style.setProperty("--subtitle-top", `${cssY}px`);
  subtitleLinkEl.style.setProperty("--subtitle-size", `${cssFontSize}px`);
}
...
function drawBackground() {
  ...
  const titleTopY = h * 0.34; /* nâng tiêu đề lên cao hơn so với trước (0.38) */
  bgCtx.font = `700 ${titleSize}px 'Space Grotesk', sans-serif`;
  bgCtx.fillText("Focal", w * 0.5, titleTopY);
  bgCtx.fillText("Living", w * 0.5, titleTopY + titleSize * 1.05);

  /* subtitle: chỉ tính vị trí/cỡ chữ ở đây để đồng bộ với thẻ <a> HTML thật,
     không vẽ trực tiếp lên canvas (để có thể click được) */
  const subSize = Math.round(w * 0.022);
  const subtitleY = titleTopY + titleSize * 2.3;
  syncSubtitleLink(subtitleY, subSize);
